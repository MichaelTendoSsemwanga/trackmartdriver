...
class LoginPageState extends State<LoginPage> {
                    ...
                    child: Column(
                        ...
                        children: [
                                  ...
                                  children: <Widget>[
                                    ...
                                    Row(
                                      ...
                                      children: <Widget>[
                                        ...
                                        Text('I agree to Trackmart Driver '),
                                        InkWell(
                                          ...
                                          onTap: () async {
                                            if (await canLaunch(
                                                'http://sandtrackdriverapp.com/legal')) {
                                              await launch(
                                                  'http://sandtrackdriverapp.com/legal');
                                            } else {
                                              print(
                          ...
                          Row(
                            ...
                            children: <Widget>[
                              Text(
                                  '${login ? "New to Trackmart Driver?" : "Already have an account?"}'),
                              InkWell(
  ...
  saveUserDetails(FirebaseUser user) async {
    ...
    login = !(fRef.collection('drivers').document(user.uid)==null);
    if(login){
      print('old');
    Map<String, dynamic> map =
    (await fRef.collection('drivers').document(user.uid)?.get()).data;
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('id', user.uid);
    ...
    await prefs.setString('phoneNo', map!=null?map['phoneNo']:(phoneNo!=null?_countryCode+phoneNo:null));
      ...
      await fRef.collection('drivers').document(user.uid).setData({
        'id': user.uid,
        'displayName': _name,
        'photoUrl': user.photoUrl,
        'phoneNo': (phoneNo!=null?_countryCode+phoneNo:null)
  ...
  Future<void> verifyPhone() async {
    if (login) {
      ...
      try {
        FirebaseUser user = await fAuth.signInWithEmailAndPassword(
            email: _countryCode + phoneNo + '@sandtrackdriverapp.com',
            password: _pin);
        await saveUserDetails(user);
        Navigator.of(context).pop();
        widget.onSignedIn(user);
